        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('knowledge-base.store') }}" method="POST" id="addAward"
                        enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="form-group mb-3">
                            <label for="">Function</label>
                           <select class="form-select" aria-label="Default select example" name="legal" id="legal">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('gallery.store') }}" method="POST" id="updateHeadline"
                        ...
                        <div class="form-group mb-3">
                            <label for="">Function</label>
                           <select class="form-select" aria-label="Default select example" name="legal_edit" id="legal_edit">
                               <option selected>-- Pilih --</option>
                                @foreach($funct as $f)
                                    <option value="{{ $f }}">{{ $f }}</option>
                                @endforeach
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h6 class="card-title">Knowledge Base Data</h6>
                            ...
                            <tbody>
                                @foreach ($data as $index => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <form
                                                action="{{ route('knowledge-base.destroy', ['knowledge_base' => $item->id]) }}"
                                                method="POST">
                                                @method('DELETE')
                                                {{ csrf_field() }}
        ...
        <script>
            $(document).ready(function() {
                ...
                $('.btn-edit').on('click', function() {
                    ...
                    var url =
                        "{{ route('knowledge-base.show', ['knowledge_base' => ':id']) }}";
                    var actionUrl =
                        "{{ route('knowledge-base.update', ['knowledge_base' => ':knowledge_base']) }}"
                    url = url.replace(':id', itemId)
                    actionUrl = actionUrl.replace(":knowledge_base", itemId)
