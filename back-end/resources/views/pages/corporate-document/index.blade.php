        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('corporate-document.store') }}" method="POST" id="addAward"
                        enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="form-group mb-3">
                            <label for="">Function</label>
                           <select class="form-select" aria-label="Default select example" name="funct">
                        ...
                        <div class="form-group mb-3">
                           ...
                           <select name="company" class="form-select" aria-label="Default select example">
                               <option selected>-- Pilih --</option>
                                @foreach($company as $companyName)
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('gallery.store') }}" method="POST" id="updateHeadline"
                        ...
                        <div class="form-group mb-3">
                            <label for="">Function</label>
                           <select class="form-select" aria-label="Default select example" name="funct_edit" id="function_edit">
                               <option selected>-- Pilih --</option>
                                @foreach($funct as $f)
                                    <option value="{{ $f }}">{{ $f }}</option>
                                @endforeach
                        ...
                        <div class="form-group mb-3">
                            <label for="">Company</label>
                           <select name="company_edit" class="form-select" aria-label="Default select example" id="company_edit">
                               <option selected>-- Pilih --</option>
                                @foreach($company as $companyName)
                                    <option value="{{ $companyName }}">{{ $companyName }}</option>
                                @endforeach
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h6 class="card-title">Corporate Document Data</h6>
                            ...
                            <tbody>
                                @foreach ($data as $index => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <form
                                                action="{{ route('corporate-document.destroy', ['corporate_document' => $item->id]) }}"
                                                method="POST">
                                                @method('DELETE')
                                                {{ csrf_field() }}
        ...
        <script>
            $(document).ready(function() {
                ...
                $('.btn-edit').on('click', function() {
                    ...
                    var url =
                        "{{ route('corporate-document.show', ['corporate_document' => ':id']) }}";
                    var actionUrl =
                        "{{ route('corporate-document.update', ['corporate_document' => ':corporate_document']) }}"
                    url = url.replace(':id', itemId)
                    actionUrl = actionUrl.replace(":corporate_document", itemId)
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $("#function_edit").val(response.function)
